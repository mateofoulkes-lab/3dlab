self.onmessage = ({data}) => {
  ...
  try {
    ...
    for (let seed=0; seed<faceCount; seed++) {
      ...
      while (head<tail) {
        ...
        for (let k=0;k<count;k++) {
          ...
          if (dotAvg>=cosLimit && dotSeed>=cosSeedLimit) {
            visited[nb]=1; queue[tail++]=nb; sumNx+=nx; sumNy+=ny; sumNz+=nz;
          }
        }
      }
      ...
      let plen=Math.hypot(pnx,pny,pnz); if (plen<1e-8) continue;
      ...
      for (let q=0;q<tail;q++) {
        ...
        for (let j=0;j<3;j++) {
          ...
          dsum+=(positions[vi]*pnx+positions[vi+1]*pny+positions[vi+2]*pnz)*w; wsum+=w;
        }
      }
      ...
      for (let q=0;q<tail;q++) {
        const f=queue[q];
        for (let j=0;j<3;j++) {
          const v=indices[f*3+j]; if (bestRegionSize[v] >= tail) continue;
          const vi=v*3;
          const dist=positions[vi]*pnx+positions[vi+1]*pny+positions[vi+2]*pnz-planeD;
